...
function wpam_wc_handle_customer_creation($user_id, $posted) {
    ...
    if(get_option(WPAM_PluginConfig::$AutoAffiliateApproveIsEnabledOption) == 1){
        $fields['userId'] = $user_id; //only set the user id when auto approval is enabled. Otherwise admin will not be able to approve and the plugin will show ERROR: User already has an account and is already an affiliate
    }
